...
Informe de las pantallas desarrolladas en la aplicación:

- **Inicio (MainActivity):** Pantalla principal. Muestra el carrusel y los listados de películas populares y en tendencia, además de dar acceso a la navegación general.
- **Iniciar Sesión (SigninActivity):** Formulario de login para ingresar con email y contraseña.
- **Registro (SignupActivity):** Pantalla para registrar un usuario nuevo ingresando nombre, email y contraseña.
- **Buscador (SearchActivity):** Permite buscar películas escribiendo el título en la barra de búsqueda.
- **Detalle de Película (MovieDetailActivity):** Muestra la info completa de la película seleccionada (sinopsis, puntuación, año, elenco) y el botón para guardarla en la lista personal.
- **Categorías (CategoriesActivity):** Muestra los diferentes géneros cinematográficos disponibles (Acción, Comedia, Drama, etc.).
- **Películas por Género (GenreMoviesActivity):** Muestra la lista de películas filtradas según el género que elegiste en Categorías.
- **Nuevos Estrenos (NewReleasesActivity):** Listado dedicado con las películas más recientes publicadas.
- **Mi Lista / Watchlist (WatchlistActivity):** Muestra las películas que el usuario guardó para ver después, esta las trae de firestore, debe estar registrado/haber iniciado sesion.
- **Mi Cuenta (AccountActivity):** Pantalla de perfil donde se ven los datos del usuario actual y la opción para cerrar sesión.
...
| `com.google.firebase:firebase-auth`      | _Gestionada_ | Autenticación de usuarios (registro, login y logout).                                  |
| `com.google.firebase:firebase-firestore` | _Gestionada_ | Base de datos NoSQL en la nube para sincronizar la Watchlist por usuario.              |
